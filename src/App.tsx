...
function App() {
  ...
  const [points, setPoint] = useState(75300000);
  const [clicks, setClicks] = useState<{ id: Number; x: Number; y: number }[]>(
    []
  );
  const pointsToAdd = 11;
  const profitPerHour = 6250000;
  ...
  const formatProfitPerHour = (profit: number) => {
    if (profit >= 1_000_000_000) {
      return `+${(profit / 1_000_000_000).toFixed(2).replace(/\.00$/, "")}B`;
    } else if (profit >= 1_000_000) {
      return `+${(profit / 1_000_000).toFixed(2).replace(/\.00$/, "")}M`;
    } else if (profit >= 1_000) {
      return `+${(profit / 1_000).toFixed(2).replace(/\.00$/, "")}K`;
    } else {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setPoint((prevPoints) => prevPoints + pointsPerSecond);
    }, 1000);
    return () => clearInterval(interval);
  }, [profitPerHour]);
      ...
      <div className="w-full bg-black text-white h-screen font-bold flex-col max-w-xl">
        <div className="px-4 z-10 ">
          ...
          <div className="flex items-center justify-between space-x-4 mt-1 ">
            ...
            <div className="border-2 w-2/3 flex items-center border-[#43433b] rounded-full px-4 py-[2px] bg-[#43433b]/[0.6] max-w-64">
              <img src={binanceLogo} alt="Exchange" className="w-8 h-8" />
              <div className="flex-1 text-center">
                <p className="text-xs text-[#85827d] font-600 pb-[2px]">
                  Profit per hour
                </p>
        ...
        <div className="flex-grow  h-full mt-4 bg-[#f3ba2f] rounded-t-[48px] relative top-glow z-0">
          <div className="absolute top-[2px] left-0 right-0 bottom-0 bg-[#1d2025] rounded-t-[46px]">
            ...
            <div className="flex justify-center px-4 mt-4">
              <div
                className="circle-outer rounded-full w-80 h-80 p-4"
